...
@pytest.fixture
def sample_applicant_data(sample_person_base_data: Dict[str, Any], sample_address_data: Dict[str, Any]) -> Dict[str, Any]:
...
@pytest.fixture
def sample_inventor_data(sample_person_base_data: Dict[str, Any], sample_address_data: Dict[str, Any]) -> Dict[str, Any]:
    data = sample_person_base_data.copy()
    data.update({
...
class TestDocumentDownloadFormat:
    ...
    def test_document_download_format_from_dict(self, sample_document_download_format_data: Dict[str, Any]) -> None:
        fmt = DocumentDownloadFormat.from_dict(sample_document_download_format_data)
        assert fmt.mime_type_identifier == sample_document_download_format_data["mimeTypeIdentifier"]
        assert fmt.download_url == sample_document_download_format_data["downloadURI"]
        assert fmt.page_total_quantity == sample_document_download_format_data["pageTotalQuantity"]

    def test_document_download_format_to_dict(self, sample_document_download_format_data: Dict[str, Any]) -> None:
        fmt = DocumentDownloadFormat(
            mime_type_identifier=sample_document_download_format_data["mimeTypeIdentifier"],
            download_url=sample_document_download_format_data["downloadURI"],
            page_total_quantity=sample_document_download_format_data["pageTotalQuantity"],
        )
        data = fmt.to_dict()
...
class TestDocument:
    ...
    def test_document_from_dict_basic(self):
        data = {
            ...
            "downloadOptionBag": [
                {"mimeTypeIdentifier": "application/pdf", "downloadURI": "url1", "pageTotalQuantity": 10}
            ]
        }
    ...
    def test_document_to_dict_basic(self):
        doc = Document(
            ...
            download_formats=[DocumentDownloadFormat(mime_type_identifier="image/tiff", page_total_quantity=5)]
        )
        data = doc.to_dict()
...
class TestAddress:
    ...
    def test_address_to_dict(self, sample_address_data: Dict[str, Any]) -> None:
        address = Address(**{
            ''.join(['_' + i.lower() if i.isupper() else i for i in k]).lstrip('_'): v
    ...
    def test_address_to_dict_empty(self):
        ...
        expected_camel_case_empty_dict = {
            ...
            "addressLineTwoText": None, "addressLineThreeText": None, "addressLineFourText": None,
            "geographicRegionName": None, "geographicRegionCode": None, "postalCode": None,
            "cityName": None, "countryCode": None, "countryName": None,
            "postalAddressCategory": None, "correspondentNameText": None
        }
...
class TestTelecommunication:
    ...
    def test_telecommunication_from_dict(self, sample_telecommunication_data: Dict[str, Any]) -> None:
        telecom = Telecommunication.from_dict(sample_telecommunication_data)
        assert telecom.telecommunication_number == sample_telecommunication_data["telecommunicationNumber"]
        assert telecom.extension_number == sample_telecommunication_data["extensionNumber"]
        assert telecom.telecom_type_code == sample_telecommunication_data["telecomTypeCode"]

    def test_telecommunication_to_dict(self, sample_telecommunication_data: Dict[str, Any]) -> None:
        telecom = Telecommunication(
            telecommunication_number=sample_telecommunication_data["telecommunicationNumber"],
...
class TestApplicant:
    ...
    def test_applicant_from_dict(self, sample_applicant_data: Dict[str, Any]) -> None:
        ...
        assert applicant.applicant_name_text == sample_applicant_data["applicantNameText"]
        assert len(applicant.correspondence_address_bag) == 1
        assert applicant.correspondence_address_bag[0].city_name == sample_applicant_data["correspondenceAddressBag"][0]["cityName"]

    def test_applicant_to_dict(self, sample_applicant_data: Dict[str, Any], sample_address_data: Dict[str, Any]) -> None:
        applicant = Applicant(
        ...
        assert data_dict["applicantNameText"] == sample_applicant_data["applicantNameText"]
...
class TestInventor:
    ...
    def test_inventor_from_dict(self, sample_inventor_data: Dict[str, Any]) -> None:
        ...
        assert inventor.correspondence_address_bag[0].city_name == sample_inventor_data["correspondenceAddressBag"][0]["cityName"]

    def test_inventor_to_dict(self, sample_inventor_data: Dict[str, Any], sample_address_data: Dict[str, Any]) -> None:
        inventor = Inventor(
...
class TestAttorney:
    ...
    def test_attorney_from_dict(self, sample_attorney_data: Dict[str, Any]) -> None:
        ...
        assert attorney.registration_number == sample_attorney_data["registrationNumber"]
        assert attorney.active_indicator == sample_attorney_data["activeIndicator"]
        assert len(attorney.attorney_address_bag) == 1
        assert attorney.attorney_address_bag[0].city_name == sample_attorney_data["attorneyAddressBag"][0]["cityName"]
        assert len(attorney.telecommunication_address_bag) == 1
        assert attorney.telecommunication_address_bag[0].telecommunication_number == sample_attorney_data["telecommunicationAddressBag"][0]["telecommunicationNumber"]

    def test_attorney_to_dict(self, sample_attorney_data: Dict[str, Any], sample_address_data: Dict[str, Any], sample_telecommunication_data: Dict[str, Any]) -> None:
        attorney = Attorney(
            ...
            registered_practitioner_category=sample_attorney_data["registeredPractitionerCategory"],
            attorney_address_bag=[Address.from_dict(sample_address_data)],
            telecommunication_address_bag=[Telecommunication.from_dict(sample_telecommunication_data)]
        )
        data_dict = attorney.to_dict()
        assert data_dict["firstName"] == sample_attorney_data["firstName"]
        assert data_dict["registrationNumber"] == sample_attorney_data["registrationNumber"]
        assert len(data_dict["attorneyAddressBag"]) == 1
        assert data_dict["attorneyAddressBag"][0]["cityName"] == sample_address_data["cityName"]
...
class TestEntityStatus:
    ...
    def test_entity_status_to_dict(self) -> None:
        entity_status = EntityStatus(small_entity_status_indicator=False, business_entity_status_category="LARGE")
...
class TestCustomerNumberCorrespondence:
    ...
    def test_customer_number_correspondence_from_dict(self, sample_address_data: Dict[str, Any], sample_telecommunication_data: Dict[str, Any]) -> None:
        data = {
        ...
        assert cust_corr.power_of_attorney_address_bag[0].city_name == sample_address_data["cityName"]
        assert len(cust_corr.telecommunication_address_bag) == 1
        assert cust_corr.telecommunication_address_bag[0].telecom_type_code == sample_telecommunication_data["telecomTypeCode"]

    def test_customer_number_correspondence_to_dict(self, sample_address_data: Dict[str, Any], sample_telecommunication_data: Dict[str, Any]) -> None:
...
class TestRecordAttorney:
    ...
    def test_record_attorney_from_dict(self, sample_attorney_data: Dict[str, Any]) -> None:
        data = {
            "customerNumberCorrespondenceData": [
        ...
        assert record_attorney.customer_number_correspondence_data[0].patron_identifier == 12345
        assert len(record_attorney.power_of_attorney_bag) == 1
        assert record_attorney.power_of_attorney_bag[0].first_name == sample_attorney_data["firstName"]
        assert len(record_attorney.attorney_bag) == 1
        assert record_attorney.attorney_bag[0].registration_number == sample_attorney_data["registrationNumber"]

    def test_record_attorney_to_dict(self, sample_attorney_data: Dict[str, Any]) -> None:
...
class TestAssignee:
    ...
    def test_assignee_to_dict(self, sample_address_data: Dict[str, Any]) -> None:
        ...
        assignee = Assignee(assignee_name_text="Another Co.", assignee_address=address_obj)
...
class TestAssignment:
    ...
    def test_assignment_from_dict(self, sample_address_data: Dict[str, Any]) -> None:
        data = {
            ...
            "conveyanceText": "ASSIGNMENT OF ASSIGNORS INTEREST",
            "assignorBag": [{"assignorName": "John Smith", "executionDate": "2022-12-15"}],
            "assigneeBag": [{"assigneeNameText": "Test Company Inc.", "assigneeAddress": sample_address_data}],
            "correspondenceAddressBag": [sample_address_data],
        }
        ...
        assert assignment.correspondence_address_bag[0].city_name == sample_address_data["cityName"]
    ...
    def test_assignment_to_dict(self, sample_address_data: Dict[str, Any]) -> None:
        ...
        assignee_obj = Assignee(assignee_name_text="Recipient", assignee_address=address_obj)
...
class TestParentContinuity:
    ...
    def test_parent_continuity_from_dict(self, sample_parent_continuity_data: Dict[str, Any]) -> None:
    ...
    def test_parent_continuity_to_dict(self, sample_parent_continuity_data: Dict[str, Any]) -> None:
        pc_instance = ParentContinuity(
            first_inventor_to_file_indicator=sample_parent_continuity_data["firstInventorToFileIndicator"],
            parent_application_status_code=sample_parent_continuity_data["parentApplicationStatusCode"],
            parent_patent_number=sample_parent_continuity_data["parentPatentNumber"],
            parent_application_status_description_text=sample_parent_continuity_data["parentApplicationStatusDescriptionText"],
            parent_application_filing_date=parse_to_date(sample_parent_continuity_data["parentApplicationFilingDate"]),
            parent_application_number_text=sample_parent_continuity_data["parentApplicationNumberText"],
            child_application_number_text=sample_parent_continuity_data["childApplicationNumberText"],
            claim_parentage_type_code=sample_parent_continuity_data["claimParentageTypeCode"],
            claim_parentage_type_code_description_text=sample_parent_continuity_data["claimParentageTypeCodeDescriptionText"],
             application_number_text=sample_parent_continuity_data["parentApplicationNumberText"],
             filing_date=parse_to_date(sample_parent_continuity_data["parentApplicationFilingDate"]),
             status_code=sample_parent_continuity_data["parentApplicationStatusCode"],
             status_description_text=sample_parent_continuity_data["parentApplicationStatusDescriptionText"],
...
class TestChildContinuity:
    ...
    def test_child_continuity_from_dict(self, sample_child_continuity_data: Dict[str, Any]) -> None:
    ...
    def test_child_continuity_to_dict(self, sample_child_continuity_data: Dict[str, Any]) -> None:
        cc_instance = ChildContinuity(
            first_inventor_to_file_indicator=sample_child_continuity_data["firstInventorToFileIndicator"],
            child_application_status_code=sample_child_continuity_data["childApplicationStatusCode"],
            parent_application_number_text=sample_child_continuity_data["parentApplicationNumberText"],
            child_application_number_text=sample_child_continuity_data["childApplicationNumberText"],
            child_application_status_description_text=sample_child_continuity_data["childApplicationStatusDescriptionText"],
            child_application_filing_date=parse_to_date(sample_child_continuity_data["childApplicationFilingDate"]),
            child_patent_number=sample_child_continuity_data["childPatentNumber"],
            claim_parentage_type_code=sample_child_continuity_data["claimParentageTypeCode"],
            claim_parentage_type_code_description_text=sample_child_continuity_data["claimParentageTypeCodeDescriptionText"],
            application_number_text=sample_child_continuity_data["childApplicationNumberText"],
            filing_date=parse_to_date(sample_child_continuity_data["childApplicationFilingDate"]),
            status_code=sample_child_continuity_data["childApplicationStatusCode"],
            status_description_text=sample_child_continuity_data["childApplicationStatusDescriptionText"],
...
class TestPatentTermAdjustmentHistoryData:
    ...
    def test_pta_history_from_dict(self) -> None:
        data = {
            "eventDate": "2022-01-01", "applicantDayDelayQuantity": 10.0,
            "eventDescriptionText": "Response to Office Action", "eventSequenceNumber": 1.0,
            "ipOfficeDayDelayQuantity": 5.0, "originatingEventSequenceNumber": 0.0,
            "ptaPTECode": "A",
        }
    ...
    def test_pta_history_to_dict(self) -> None:
        pta_hist = PatentTermAdjustmentHistoryData(
            event_date=date(2022,1,1), applicant_day_delay_quantity=10.0,
            event_description_text="Response to Office Action", event_sequence_number=1.0,
            ip_office_day_delay_quantity=5.0, originating_event_sequence_number=0.0,
            pta_pte_code="A"
        )
...
class TestPatentTermAdjustmentData:
    ...
    def test_pta_data_from_dict(self) -> None:
        data = {
            "aDelayQuantity": 100.0, "adjustmentTotalQuantity": 150.0,
            "filingDate": "2020-01-01", "grantDate": "2023-01-01",
        ...
        assert pta_data.patent_term_adjustment_history_data_bag[0].event_date == date(2022,1,1)
    ...
    def test_pta_data_to_dict(self) -> None:
        ...
        pta_data = PatentTermAdjustmentData(
            a_delay_quantity=100.0, filing_date=date(2020,1,1),
...
class TestEventData:
    ...
    def test_event_data_from_dict(self) -> None:
        data = {"eventCode": "COMP", "eventDescriptionText": "Ready", "eventDate": "2022-01-01"}
    ...
    def test_event_data_to_dict(self) -> None:
        event = EventData(event_code="MAIL", event_description_text="Mailed", event_date=date(2022,2,2))
        data = event.to_dict()
...
class TestDocumentMetaData:
    ...
    def test_document_meta_data_from_dict(self, sample_document_meta_data_data: Dict[str, Any]) -> None:
        ...
        assert doc_meta.file_create_date_time == datetime(2023,1,1,12,0,0, tzinfo=timezone.utc)

    def test_document_meta_data_to_dict(self, sample_document_meta_data_data: Dict[str, Any]) -> None:
...
class TestApplicationMetaData:
    ...
    def test_application_meta_data_from_dict(self) -> None:
        data = {
            "nationalStageIndicator": True,
            "entityStatusData": {"smallEntityStatusIndicator": True, "businessEntityStatusCategory": "SMALL"},
            "publicationDateBag": ["2022-01-01", "invalid-date", None], # Test mixed valid/invalid/None dates
            "firstInventorToFileIndicator": "Y",
        ...
        assert app_meta.publication_sequence_number_bag == ["1", None, "2"] # Nones preserved in simple lists
    ...
    def test_application_meta_data_to_dict(self) -> None: 
        ...
        assert data_with_class == {"class": "XYZ"} # Test "class_field" to "class" mapping

        app_meta_with_class_none = ApplicationMetaData(class_field=None) # class_field is None
        data_with_class_none = app_meta_with_class_none.to_dict()
        assert "class" not in data_with_class_none # class_field was None, so 'class' key is not added

        # Test with an empty list to hit line 1158
        app_meta_empty_cpc = ApplicationMetaData(invention_title="Test", cpc_classification_bag=[])
...
class TestPatentFileWrapper:
    ...
    def test_patent_file_wrapper_from_dict(self, sample_document_meta_data_data: Dict[str, Any]) -> None:
        data = {
            "applicationNumberText": "12345678",
            "applicationMetaData": {"inventionTitle": "Test Invention", "filingDate": "2020-01-01"},
        ...
        assert wrapper.pgpub_document_meta_data.zip_file_name == sample_document_meta_data_data["zipFileName"]
        assert wrapper.last_ingestion_date_time == datetime(2023,1,1,10,0,0, tzinfo=timezone.utc)

    def test_patent_file_wrapper_to_dict(self, sample_document_meta_data_data: Dict[str, Any]) -> None:
        ...
        wrapper = PatentFileWrapper(
            ...
            last_ingestion_date_time=datetime(2023,2,2,11,0,0,tzinfo=timezone.utc)
        )
        data = wrapper.to_dict()
        assert data["applicationNumberText"] == "98765"
        assert data["applicationMetaData"]["inventionTitle"] == "Title"
        assert data["pgpubDocumentMetaData"]["zipFileName"] == sample_document_meta_data_data["zipFileName"]
        assert data["lastIngestionDateTime"] == "2023-02-02T11:00:00Z"

    def test_patent_file_wrapper_with_grant_document_meta_data(self, sample_document_meta_data_data: Dict[str, Any]) -> None:
        data = {
            "applicationNumberText": "12345678",
        ...
        assert wrapper.grant_document_meta_data.zip_file_name == sample_document_meta_data_data["zipFileName"]
...
class TestPatentDataResponse:
    ...
    def test_patent_data_response_to_dict(self) -> None:
        ...
        response = PatentDataResponse(count=2, patent_file_wrapper_data_bag=[wrapper1, wrapper2])
    ...
    def test_patent_data_response_to_dict_with_sample(self, patent_data_sample: Dict[str, Any]) -> None:
        ...
        assert len(result["patentFileWrapperDataBag"]) == len(response.patent_file_wrapper_data_bag)
        if response.count > 0:
            assert result["patentFileWrapperDataBag"][0]["applicationNumberText"] == \
...
class TestStatusCode:
    ...
    def test_status_code_from_dict(self) -> None:
        data = {"applicationStatusCode": 101, "applicationStatusDescriptionText": "Status Description"}
    ...
    def test_status_code_to_dict(self) -> None:
        ...
        data = sc.to_dict()
        assert data == {"applicationStatusCode": 101, "applicationStatusDescriptionText": "Status Description"}
...
class TestStatusCodeCollection:
    ...
    def test_status_code_collection_repr(self): # Target line 1392
        ...
        assert repr(collection_long) == "StatusCodeCollection(4 status codes: 1, 2, 3, ...)"
...
class TestStatusCodeSearchResponse:
    ...
    def test_status_code_search_response_to_dict(self) -> None:
        ...
        code2 = StatusCode(code=102, description="Status 2")
        collection = StatusCodeCollection([code1, code2])
        response = StatusCodeSearchResponse(count=2, status_code_bag=collection, request_identifier="req123")
...
class TestApplicationContinuityData:
    ...
    def test_to_dict(self, sample_parent_continuity_data, sample_child_continuity_data):
        ...
        assert data_dict["parentContinuityBag"][0]["parentApplicationNumberText"] == sample_parent_continuity_data["parentApplicationNumberText"]
        assert data_dict["childContinuityBag"][0]["childApplicationNumberText"] == sample_child_continuity_data["childApplicationNumberText"]
